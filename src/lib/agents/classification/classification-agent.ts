...
const ClassificationInputSchema = z.object({
  ...
  analysisDepth: z.enum(['quick', 'thorough', 'comprehensive']).optional().default('thorough')
});

export class ClassificationAgent extends BaseAgent<ClassificationInput, ClassificationOutput> {
      ...
      capabilities: [
        {
          ...
          description: 'Detect relevant compliance frameworks from project description and documents',
          inputSchema: ClassificationInputSchema,
          outputSchema: z.object({
            detectedFrameworks: z.array(z.object({
              name: z.string(),
              confidence: z.number(),
  ...
  protected createPrompt(): ChatPromptTemplate {
    return ChatPromptTemplate.fromMessages([
      ['system', `You are a compliance framework classification expert. Your role is to analyze project descriptions and documents to identify relevant compliance frameworks.
...
RESPOND WITH: Structured analysis including detected frameworks, confidence scores, reasoning, and recommendations.`],
  ...
  protected async preprocessInput(input: AgentInput<ClassificationInput>): Promise<AgentInput<ClassificationInput>> {
    // Validate input schema
    const validatedData = ClassificationInputSchema.parse(input.data);
    return { ...input, data: validatedData };
  }

  protected async postprocessOutput(result: any, input: AgentInput<ClassificationInput>): Promise<ClassificationOutput> {
    ...
    try {
      ...
      if (detectedFrameworks.length === 0) {
        console.log('AI response was empty or failed, using keyword fallback for classification');
      ...
      const overallAssessment = this.performOverallAssessment(detectedFrameworks, input.data);

      // Get metadata from intermediate steps
      const analysisMetadata = this.extractAnalysisMetadata(result.intermediateSteps || []);
  ...
  protected formatInputForAgent(input: AgentInput<ClassificationInput>): string {
    const { projectDescription, documentContent, existingFrameworks, analysisDepth } = input.data;
...
Focus on frameworks most relevant to the project context and regulatory environment.`;
  ...
  private async parseAIResponse(aiResponse: string): Promise<ClassificationOutput['detectedFrameworks']> {
    try {
      ...
      const frameworkNames = ['FERPA', 'HIPAA', 'GDPR', 'COPPA', 'IRB', 'SOC 2', 'ISO 27001', 'Section 508', 'ITAR', 'EAR', 'CCPA', 'PIPEDA'];
      ...
      for (const frameworkName of frameworkNames) {
        ...
        if (match) {
          ...
          const confidenceMatch = aiResponse.match(new RegExp(`${frameworkName}[^.]*confidence[^.]*?(\\d+(?:\\.\\d+)?)`, 'i'));
          const confidence = confidenceMatch ? parseFloat(confidenceMatch[1]) / 100 : 0.8;

          // Extract priority if mentioned
          const priorityMatch = aiResponse.match(new RegExp(`${frameworkName}[^.]*?(critical|high|medium|low)`, 'i'));
          const priority = (priorityMatch?.[1]?.toLowerCase() as 'critical' | 'high' | 'medium' | 'low') || 'medium';
      ...
      return frameworks.sort((a, b) => (b.confidence * b.relevanceScore) - (a.confidence * a.relevanceScore));
    } catch (error) {
  ...
  private getFrameworkRequirements(frameworkName: string): string[] {
    ...
    return frameworkDatabase[frameworkName]?.keyRequirements || ['Review compliance requirements'];
  }
  ...
  private async extractFrameworksFromResponse(
    ...
    const combinedContent = `${projectDescription} ${documentContent}`.toLowerCase();

    for (const [frameworkName, frameworkInfo] of Object.entries(frameworkDatabase)) {
      ...
      if (matches.length > 0) {
        ...
        const relevanceScore = this.calculateRelevanceScore(matches, frameworkInfo, combinedContent);

        detectedFrameworks.push({
          name: frameworkName,
          confidence,
          relevanceScore,
          reasoning: `Detected based on: ${matches.join(', ')}. ${frameworkInfo.description}`,
          requirements: frameworkInfo.keyRequirements,
          priority: this.determinePriority(confidence, relevanceScore, frameworkInfo.riskLevel)
        });
      }
    }
  ...
  private getFrameworkDatabase(): Record<string, any> {
    return {
      'FERPA': {
        description: 'Family Educational Rights and Privacy Act - protects student educational records',
        triggers: ['student', 'education', 'school', 'university', 'academic', 'grade', 'transcript', 'enrollment'],
        keyRequirements: ['Student consent', 'Directory information policies', 'Access controls', 'Audit logs'],
        riskLevel: 'high'
      },
      'HIPAA': {
        description: 'Health Insurance Portability and Accountability Act - protects health information',
        triggers: ['health', 'medical', 'patient', 'healthcare', 'clinical', 'diagnosis', 'treatment'],
        keyRequirements: ['Data encryption', 'Access controls', 'Audit logs', 'Business associate agreements'],
        riskLevel: 'critical'
      },
      'GDPR': {
        description: 'General Data Protection Regulation - EU data protection law',
        triggers: ['personal data', 'eu', 'europe', 'privacy', 'consent', 'data subject'],
        keyRequirements: ['Lawful basis', 'Consent management', 'Data subject rights', 'Privacy by design'],
        riskLevel: 'high'
      },
      'COPPA': {
        description: 'Children\'s Online Privacy Protection Act - protects children under 13',
        triggers: ['children', 'kids', 'minor', 'under 13', 'parental', 'child'],
        keyRequirements: ['Parental consent', 'Data minimization', 'Safe harbor', 'Age verification'],
        riskLevel: 'critical'
      },
      'IRB': {
        description: 'Institutional Review Board - human subjects research oversight',
        triggers: ['research', 'human subjects', 'study', 'participant', 'survey', 'experiment'],
        keyRequirements: ['IRB approval', 'Informed consent', 'Risk assessment', 'Data security'],
        riskLevel: 'high'
      },
      'SOC 2': {
        description: 'Service Organization Control 2 - security controls for service providers',
        triggers: ['service provider', 'cloud', 'security controls', 'audit', 'third party'],
        keyRequirements: ['Security controls', 'Monitoring', 'Risk assessment', 'Vendor management'],
        riskLevel: 'medium'
      },
      'ISO 27001': {
        description: 'Information Security Management System standard',
        triggers: ['information security', 'isms', 'security management', 'iso'],
        keyRequirements: ['Security policy', 'Risk management', 'Access controls', 'Incident response'],
        riskLevel: 'medium'
      },
      'Section 508': {
        description: 'Federal accessibility requirements for digital content',
        triggers: ['accessibility', 'disability', 'ada', 'wcag', 'federal', 'government'],
        keyRequirements: ['WCAG compliance', 'Alt text', 'Keyboard navigation', 'Screen reader support'],
        riskLevel: 'medium'
      }
    };
  }

  private calculateRelevanceScore(matches: string[], frameworkInfo: any, content: string): number {
  ...
  private determinePriority(confidence: number, relevance: number, riskLevel: string): 'critical' | 'high' | 'medium' | 'low' {
  ...
  private getMostCommonDomain(domains: string[]): string {
    ...
    return Object.entries(counts).reduce((a, b) => a[1] > b[1] ? a : b)[0] || 'general';
  }

  private mapPriorityToRisk(priority: string): 'low' | 'medium' | 'high' | 'critical' {
  ...
  private extractAnalysisMetadata(intermediateSteps: any[]): Omit<ClassificationOutput['analysisMetadata'], 'processingTime'> {
  ...
  private createFallbackResponse(input: ClassificationInput): ClassificationOutput {
    return {
      detectedFrameworks: [{
        ...
        requirements: ['Review applicable regulations', 'Implement basic security controls'],
        priority: 'medium'
      }],
      overallAssessment: {
